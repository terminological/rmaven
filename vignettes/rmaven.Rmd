...
sprintf("Jar files found in repository: %1.0f", fs::dir_ls(get_repository_location(),recurse = TRUE,glob="*.jar") %>% length())
```
...
```{r}
execute_maven("help:system",quiet = FALSE)
```

# Options and configuration

Various options exist to control behaviour of the library

```R
# Set the level of information returned by Maven. Maven can be very verbose when downloading a lot
# of content so rmaven defaults to "quiet" mode
options("rmaven.quiet"=FALSE)

# Set the default repository URLs for fetching artifacts.
# these can also be specified as options for the fetch_artifact() and copy_artifact() functions.
...
# Manually set JAVA_HOME for Maven to use. 
# The home directory of the Java installation is detected automatically by firstly checking this option, 
# then the JAVA_HOME enviroment variable, then the LD_LIBRARY_PATH environment variable, then asking 'rJava'.
...
# the following option has the dual effect of:
# starting the JVM in debug mode (JVM flags: -Xdebug -Xrunjdwp:transport=dt_socket,address=8998,server=y,suspend=n ), and 
# asking Maven to print debugging output (Maven flags: -X -e)
...
# If you using rmaven in a package you intend to submit to CRAN this will invoke their ire.
...
# this will involve a lot of downloading from maven central.
# generally only required on a development machine where you want to make sure everything runs from scratch
